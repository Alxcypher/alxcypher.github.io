...
(function () {
  ...
  function isSizeChartPage() {
    return (
      ...
      window.location.pathname.includes('size_chart') ||
      window.location.pathname.includes('/help/size')
    );
  }

  function isProductPage() {
    return (
      document.querySelector('[data-auto-id="product-title"]') !== null ||
      document.querySelector('.product-description__title') !== null ||
  ...
  function scrapeSizeChart() {
    ...
    // Adidas uses both standard tables and custom React components
    const tables = document.querySelectorAll(
      '.size-chart__table table, [data-auto-id="size-chart-table"], table'
    );
    ...
    // Adidas also renders size charts as div grids with data attributes
    if (results.length === 0) {
      const gridRows = document.querySelectorAll(
        '[data-auto-id="size-chart-row"], [class*="size-chart"] [class*="row"]'
      );
      for (const row of gridRows) {
        const cells = row.querySelectorAll(
          '[data-auto-id="size-chart-cell"], [class*="cell"], span'
        );
        if (cells.length >= 2) {
          const entry = {};
  ...
  function scrapeProductInfo() {
    const title =
      document.querySelector('[data-auto-id="product-title"]')?.textContent ||
      document.querySelector('.product-description__title')?.textContent ||
  ...
  function detectGender() {
    ...
    // Adidas uses query params sometimes
    const params = new URLSearchParams(window.location.search);
    const gender = params.get('gender') || '';
    if (gender.toLowerCase().includes('women')) return 'womens';
