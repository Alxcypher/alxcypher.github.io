...
(function () {
  ...
  function scrapeSizeChart() {
    ...
    const tables = document.querySelectorAll(
      '.size-chart table, .pdp-size-chart table, table'
    );
    ...
    // NB also uses div-based size grids on their newer site
    if (results.length === 0) {
      const gridRows = document.querySelectorAll(
        '[class*="size-chart"] [class*="row"], [class*="SizeChart"] [class*="Row"]'
      );
  ...
  function scrapeProductInfo() {
    ...
    const trailModels = ['hierro', 'nitrel', 'summit unknown', 'fresh foam x trail'];
    const category = trailModels.some((m) => title.toLowerCase().includes(m)) ? 'trail' : 'road';

    const modelName = title.replace(/New\s+Balance\s*/i, '').trim();
