/**
 * SizeCompare — Puma Content Script
 *
 * Scrapes size chart data from puma.com pages.
 */
...
(function () {
  ...
  function isProductPage() {
    return (
      document.querySelector('[data-test-id="product-title"]') !== null ||
  ...
  function scrapeSizeChart() {
    ...
    const tables = document.querySelectorAll(
      '.size-chart table, [data-testid="size-chart"] table, table'
    );
  ...
  function scrapeProductInfo() {
    const title =
      document.querySelector('[data-test-id="product-title"]')?.textContent ||
      document.querySelector('.product-name')?.textContent ||
      document.querySelector('h1')?.textContent ||
      '';

    const trailModels = ['voyage nitro', 'explore nitro'];
    const category = trailModels.some((m) => title.toLowerCase().includes(m)) ? 'trail' : 'road';

    const modelName = title.replace(/PUMA\s*/i, '').trim();
